...
class PromptModel():
    ...
    def calibrate(self, beam_size, restrict_ans=["Yes", "No"], max_len=2, calib_prompt="Yes or No?"):
        if self.config.task_name in ['coref']:
            def restrict_decode_vocab(batch_idx, prefix_beam):
                return \
                self.tokenizer(restrict_ans, add_special_tokens=True, return_tensors="pt", is_split_into_words=True)[
                    'input_ids'].tolist()

            calib_out = self.model.generate(self.tokenizer(calib_prompt, return_tensors="pt").input_ids.to(device),
                                            num_return_sequences=beam_size, num_beams=beam_size, output_scores=True,
                                            return_dict_in_generate=True,
                                            prefix_allowed_tokens_fn=restrict_decode_vocab, max_length=max_len)
            gens = []
            ans_scores = []
            ans_order = []
            ...
            for seq_ix in range(beam_size):
                seq_dict = {}
                seq_dict["sentence"] = self.tokenizer.decode(calib_out.sequences[seq_ix],
    ...
    def generate(self, beam_size=1, test_mode=False, do_calibrate=False):
        ...
        restriction, max_len, calib_prompt = restrict_vocab(
            self.config)  # Restrictions on generation vocabulary and dummy prompts

        def restrict_decode_vocab(batch_idx, prefix_beam):
            """ Function to restrict decode vocab to some tokens. Source: https://github.com/huggingface/transformers/issues/15169
            """
            return self.tokenizer(restriction, add_special_tokens=True, return_tensors="pt", is_split_into_words=True)[
                'input_ids'].tolist()
        ...
        if do_calibrate:
            calib_out, calib_order = self.calibrate(beam_size, restrict_ans=restriction.copy(), max_len=max_len,
        ...
        for ix, prompt in tqdm(enumerate(prompts)):
            ...
            with torch.no_grad():
                if self.config.task_name in ['coref']:
                    outputs = self.model.generate(input_ids.to(device), num_return_sequences=beam_size,
                                                  num_beams=beam_size, output_scores=True, return_dict_in_generate=True,
                                                  prefix_allowed_tokens_fn=restrict_decode_vocab, max_length=max_len)
                else:
                    outputs = self.model.generate(input_ids.to(device), num_return_sequences=beam_size,
                                                  num_beams=beam_size, output_scores=True, return_dict_in_generate=True)
...
if __name__ == "__main__":
    ...
    with open(f"./../filtered_dumps/filtered_gens_vero_null.bin", "rb") as out:
        gens = pickle.load(out)
    with open(f"./../filtered_dumps/gold_vero_null.bin", "rb") as out:
        gold = pickle.load(out)
    ...
    print("Unconstrained")
    meta = {"gold_dump_file": f"./../results/coref/{dataset_name}_{file_infix}_gold.txt",
            "pred_dump_file": f"./../results/coref/{dataset_name}_{file_infix}_uncons.txt", "constrained": False}
    uncon_gens = []
    for gen in gens:
        uncon_gens.append(gen[0]["sentence"])
    ...
    print("Constrained")
    meta = {"gold_dump_file": f"./../results/coref/{dataset_name}_{file_infix}_gold.txt",
            "pred_dump_file": f"./../results/coref/{dataset_name}_{file_infix}_cons.txt", "constrained": True}
